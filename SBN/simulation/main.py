...
def get_args():
    ...
    parser.add_argument('--k_all', default=500, type=int)
    parser.add_argument('--method', default='sbn-em', choices=['sbn-em', 'sbn-sem', 'sbn-semvr', 'sbn-sga', 'sbn-svrg', 'sbn-em-a', 'sbn-sem-a', 'sbn-semvr-a', 'freq'])
...
def main(args):
    ...
    if args.method == 'sbn-em':
        ...
        save_to_path = 'results/simulation_{}/sbn-em/'.format(args.k_all)+'beta_'+str(args.beta)+'_k_'+str(args.n_trees)+'.pkl'
    if args.method == 'sbn-sem':
        model = SBN(taxa, rootsplit_supp_dict, subsplit_supp_dict, emp_tree_freq)
        ...
        results = {'logp': logp, 'kldivs': kldivs, 'size': len(tree_dict)}
        save_to_path = 'results/simulation_{}/sbn-sem/'.format(args.k_all)+'beta_'+str(args.beta)+'_k_'+str(args.n_trees)+'.pkl'
    if args.method == 'sbn-semvr':
        model = SBN(taxa, rootsplit_supp_dict, subsplit_supp_dict, emp_tree_freq)
        ...
        results = {'logp': logp, 'kldivs': kldivs, 'size': len(tree_dict)}
        save_to_path = 'results/simulation_{}/sbn-semvr/'.format(args.k_all)+'beta_'+str(args.beta)+'_k_'+str(args.n_trees)+'.pkl'
    if args.method == 'sbn-sga':
        model = SBN(taxa, rootsplit_supp_dict, subsplit_supp_dict, emp_tree_freq)
        ...
        results = {'logp': logp, 'kldivs': kldivs, 'size': len(tree_dict)}
        save_to_path = 'results/simulation_{}/sbn-sga/'.format(args.k_all)+'beta_'+str(args.beta)+'_k_'+str(args.n_trees)+'.pkl'
    if args.method == 'sbn-svrg':
        model = SBN(taxa, rootsplit_supp_dict, subsplit_supp_dict, emp_tree_freq)
        ...
        results = {'logp': logp, 'kldivs': kldivs, 'size': len(tree_dict)}
        save_to_path = 'results/simulation_{}/sbn-svrg/'.format(args.k_all)+'beta_'+str(args.beta)+'_k_'+str(args.n_trees)+'.pkl'
    if args.method == 'sbn-em-a':
        model = SBN(taxa, rootsplit_supp_dict, subsplit_supp_dict, emp_tree_freq)
        logp, kldivs = model.EM_learn(tree_dict, tree_names, tree_wts, maxiter=args.niters, miniter=args.niters, abstol=1e-05, monitor=True, start_from_uniform=False, report_kl=True, alpha=50.0/args.n_trees)
        results = {'logp': logp, 'kldivs': kldivs, 'size': len(tree_dict)}
        save_to_path = 'results/simulation_{}/sbn-em-a/'.format(args.k_all)+'beta_'+str(args.beta)+'_k_'+str(args.n_trees)+'.pkl'
    if args.method == 'sbn-sem-a':
        model = SBN(taxa, rootsplit_supp_dict, subsplit_supp_dict, emp_tree_freq)
        logp, kldivs = model.SEMVR_learn(tree_dict, tree_names, tree_wts, maxiter=args.niters, miniter=args.niters, monitor=True, ema_rate=0.999, start_from_uniform=False, biter=args.k_all, alpha=50.0/args.n_trees, report_kl=True)
        results = {'logp': logp, 'kldivs': kldivs, 'size': len(tree_dict)}
        save_to_path = 'results/simulation_{}/sbn-sem-a/'.format(args.k_all)+'beta_'+str(args.beta)+'_k_'+str(args.n_trees)+'.pkl'
    if args.method == 'sbn-semvr-a':
        model = SBN(taxa, rootsplit_supp_dict, subsplit_supp_dict, emp_tree_freq)
        logp, kldivs = model.SEMVR_learn(tree_dict, tree_names, tree_wts, maxiter=args.niters, miniter=args.niters, monitor=True, ema_rate=0.99, start_from_uniform=False, biter=args.k_all, alpha=50.0/args.n_trees, report_kl=True, vr=True)
        results = {'logp': logp, 'kldivs': kldivs, 'size': len(tree_dict)}
        save_to_path = 'results/simulation_{}/sbn-semvr-a/'.format(args.k_all)+'beta_'+str(args.beta)+'_k_'+str(args.n_trees)+'.pkl'
