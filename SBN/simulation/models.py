...
class SBN(nn.Module):
    ...
    def SEMVR_learn(self, tree_dict, tree_names, tree_wts, maxiter=100, miniter=50, abstol=1e-05, biter=1000, batch=1, ema_rate=0.99, ar=0.75, af=10,  monitor=True, vr=True, start_from_uniform=False, report_kl=False, alpha=0.0):
        self.set_clade_to_bitarr(tree_dict, tree_names)
        if start_from_uniform:
            self.CPDs = torch.ones(self.CPDParser.num_params)
            self.CPDs_normalize()
        ...
        for k in range(maxiter):
            ...
            if vr:
                for i in range(biter):
                    ...
                    prenorm_CPDs = ema_rate * prenorm_CPDs + (1-ema_rate) * (CPDs-CPDs_ctrl1 + CPDs_ctrl2) 
                    if alpha == 0.0:
    ...
    def SGA_learn(self, stepsz, tree_dict, tree_names, tree_wts, batch=1, maxiter=500, miniter=50, abstol=1e-05, biter=1000, ar=0.75, af=10, start_from_uniform=False, monitor=True, report_kl=False,vr=True):
        self.set_clade_to_bitarr(tree_dict, tree_names)
        if start_from_uniform:
